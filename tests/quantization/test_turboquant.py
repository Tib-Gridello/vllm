...
class TestKVQuantMode:
    ...
    def test_turboquant_nibble_mode(self):
        ...
        old = os.environ.get("VLLM_TURBOQUANT_BITS")
        ...
        try:
            ...
            if old is None:
                os.environ.pop("VLLM_TURBOQUANT_BITS", None)
            else:
                os.environ["VLLM_TURBOQUANT_BITS"] = old
            importlib.reload(vllm.envs)
...
class TestByteMode:
    ...
    def test_roundtrip_6bit_d128(self):
        ...
        decoded = turboquant_decode_ref(
            indices, norms, cb, torch.float32)
        cos_sim = torch.nn.functional.cosine_similarity(
            tensor, decoded, dim=1)
        assert cos_sim.mean().item() > 0.99
    ...
    def test_6bit_better_than_4bit(self):
        ...
        sim6 = torch.nn.functional.cosine_similarity(
            tensor, dec6, dim=1).mean()
...
class TestQJLEncodeDecode:
    ...
    def codebook_6bit_qjl(self):
        return TurboQuantCodebook(n_bits=6, head_dim=128, device="cpu",
                                   qjl=True)

    @pytest.fixture
    def codebook_8bit_qjl(self):
        return TurboQuantCodebook(n_bits=8, head_dim=128, device="cpu",
                                   qjl=True)
    ...
    def test_qjl_better_than_basic_6bit(self, codebook_6bit_qjl):
        ...
        # Basic TQ (no QJL)
        cb_basic = TurboQuantCodebook(n_bits=6, head_dim=128, device="cpu",
                                       qjl=False)
        idx_b, nrm_b = turboquant_encode_ref(tensor, cb_basic)
        dec_basic = turboquant_decode_ref(idx_b, nrm_b, cb_basic,
                                          torch.float32)
        sim_basic = torch.nn.functional.cosine_similarity(
            tensor, dec_basic, dim=1).mean().item()

        # QJL TQ
        idx_q, nrm_q, signs_q, rs_q = turboquant_encode_qjl_ref(
            tensor, codebook_6bit_qjl)
        dec_qjl = turboquant_decode_qjl_ref(idx_q, nrm_q, signs_q, rs_q,
                                              codebook_6bit_qjl, torch.float32)
        sim_qjl = torch.nn.functional.cosine_similarity(
            tensor, dec_qjl, dim=1).mean().item()
    ...
    def test_qjl_better_than_basic_8bit(self, codebook_8bit_qjl):
        ...
        cb_basic = TurboQuantCodebook(n_bits=8, head_dim=128, device="cpu",
                                       qjl=False)
        idx_b, nrm_b = turboquant_encode_ref(tensor, cb_basic)
        dec_basic = turboquant_decode_ref(idx_b, nrm_b, cb_basic,
                                          torch.float32)
        ...
        dec_qjl = turboquant_decode_qjl_ref(idx_q, nrm_q, signs_q, rs_q,
                                              codebook_8bit_qjl, torch.float32)
        sim_qjl = torch.nn.functional.cosine_similarity(
            tensor, dec_qjl, dim=1).mean().item()
    ...
    def test_qjl_6bit_quality_threshold(self, codebook_6bit_qjl):
        ...
        torch.manual_seed(123)
        tensor = torch.randn(200, 128)
        idx, nrm, signs, rs = turboquant_encode_qjl_ref(
            tensor, codebook_6bit_qjl)
        decoded = turboquant_decode_qjl_ref(idx, nrm, signs, rs,
                                             codebook_6bit_qjl, torch.float32)
        sim = torch.nn.functional.cosine_similarity(
            tensor, decoded, dim=1).mean().item()
    ...
    def test_qjl_inner_product_quality(self, codebook_6bit_qjl):
        ...
        idx, nrm, signs, rs = turboquant_encode_qjl_ref(
            keys, codebook_6bit_qjl)
        keys_recon = turboquant_decode_qjl_ref(idx, nrm, signs, rs,
                                                codebook_6bit_qjl,
                                                torch.float32)
        ...
        keys_basic = turboquant_decode_ref(idx_b, nrm_b, cb_basic,
                                            torch.float32)
        ...
        assert qjl_err < basic_err, (
            f"QJL ip error ({qjl_err:.4f}) should be lower than "
            f"basic ({basic_err:.4f})"
        )
    ...
    def test_qjl_res_scale_positive(self, codebook_6bit_qjl):
        ...
        torch.manual_seed(123)
        tensor = torch.randn(100, 128)
        _, _, _, res_scales = turboquant_encode_qjl_ref(
            tensor, codebook_6bit_qjl)
    ...
    def test_qjl_codebook_flag(self):
        ...
        cb6_no = TurboQuantCodebook(n_bits=6, head_dim=128, device="cpu",
                                     qjl=False)
...
class TestOutlierChannels:
    ...
    def test_outlier_custom_mask(self):
        ...
        config = OutlierChannelConfig(
            head_dim=128, outlier_bits=4, regular_bits=2,
            device="cpu", outlier_mask=mask)
        assert config.outlier_dim == 32
        assert config.regular_dim == 96
...
class TestDequantPaged:
    ...
    def _get_ref_rotated(self, cache_data):
        ...
        staging_blocks = num_seqs * max_bps
        ref_key = torch.zeros(
            staging_blocks, block_size, nkv, head_dim, device="cuda")
                ...
                for slot in range(block_size):
                    for h in range(nkv):
                        ...
                        if cb.byte_mode:
                            ...
                            k_idx = torch.cat([
                                packed_k & 0x0F,
                                (packed_k >> 4) & 0x0F], dim=-1).long()
                            packed_v = vc[:head_dim // 2]
                            v_idx = torch.cat([
                                packed_v & 0x0F,
                                (packed_v >> 4) & 0x0F], dim=-1).long()
                        ...
                        if cb.qjl:
                            ...
                            for d in range(head_dim):
                                ...
                                bit = d % 8
                                k_sign = 1.0 if (
                                    (k_sb[byt].item() >> bit) & 1) else -1.0
                                v_sign = 1.0 if (
                                    (v_sb[byt].item() >> bit) & 1) else -1.0
    ...
    def test_dequant_byte_qjl(self, codebook_6bit_qjl):
        ...
        torch.manual_seed(42)
        cd = self._build_paged_cache(codebook_6bit_qjl, num_seqs=2,
    ...
    def test_dequant_byte_no_qjl(self, codebook_6bit_no_qjl):
        ...
        torch.manual_seed(42)
        cd = self._build_paged_cache(codebook_6bit_no_qjl, num_seqs=2,
                                      seq_len=32, block_size=16, nkv=2)
        staging_key, staging_val = self._run_dequant(cd)
        ref_key, ref_val = self._get_ref_rotated(cd)
        torch.testing.assert_close(
    ...
    def test_dequant_empty_slots(self, codebook_6bit_qjl):
        ...
        cd = self._build_paged_cache(codebook_6bit_qjl, num_seqs=1,
                                      seq_len=10, block_size=16, nkv=2)
        staging_key, staging_val = self._run_dequant(cd)
                ...
                for slot in range(16):
                    for h in range(2):
                        ...
                        if expected_norm == 0.0:
                            continue
                        actual_norm = staging_key[
                            staging_blk, slot, h].float().norm().item()
                        # Should be close (not exact due to quantization)
                        assert abs(actual_norm - expected_norm) / (
                            expected_norm + 1e-8) < 0.3, (
